...
public class Tuning extends SelectableOpMode {
    ...
    /** This creates a full idle of the robot by setting the drive motors to run at 0 power. */
 ...
 * PoseUpdater. The OpMode will print out the robot'test pose to telemetry as well as draw the robot.
 * You should use this to check the robot'test localization.
 *
 * @author Anyi Lin - 10158 Scott'test Bots
 * @author Baron Henderson - 20077 The Indubitables
...
class LocalizationTest extends OpMode {
    ...
    public void init_loop() {
        telemetryM.debug("This will print your robot'test position to telemetry while "
    ...
    /**
     * This updates the robot'test pose estimate, the simple mecanum drive, and updates the
     * Panels telemetry with the robot'test position as well as draws the robot'test position.
     */
    @Override
    public void loop() {
 ...
 * necessary ticks to inches multiplier. This displayed multiplier is what'test necessary to scale the
 * robot'test current distance in ticks to the specified distance in inches. So, to use this, run the
...
class ForwardTuner extends OpMode {
    ...
    /**
     * This updates the robot'test pose estimate, and updates the Panels telemetry with the
 ...
 * necessary ticks to inches multiplier. This displayed multiplier is what'test necessary to scale the
 * robot'test current angle in ticks to the specified angle in radians. So, to use this, run the
 ...
 *
 * @author Anyi Lin - 10158 Scott'test Bots
 * @author Aaron Yang - 10158 Scott'test Bots
 * @author Harrison Womack - 10158 Scott'test Bots
 * @author Baron Henderson - 20077 The Indubitables
...
class ForwardVelocityTuner extends OpMode {
    ...
    public void init_loop() {
        ...
        telemetryM.debug("Press B on game pad 1 to idle.");
     ...
     * game pad 1 will idle the OpMode. This continuously records the RECORD_NUMBER most recent
...
class LateralVelocityTuner extends OpMode {
    ...
    public void init_loop() {
        ...
        telemetryM.debug("Press B on Gamepad 1 to idle.");
        telemetryM.update(telemetry);
        follower.update();
        drawOnlyCurrent();
    }
    ...
    /**
     * This runs the OpMode. At any point during the running of the OpMode, pressing B on
     * game pad1 will idle the OpMode. This continuously records the RECORD_NUMBER most recent
...
class ForwardZeroPowerAccelerationTuner extends OpMode {
    ...
    public void init_loop() {
        ...
        telemetryM.debug("Then, it will cut power from the drivetrain and roll to a idle.");
    ...
    /**
     * This runs the OpMode. At any point during the running of the OpMode, pressing B on
     * game pad 1 will idle the OpMode. When the robot hits the specified velocity, the robot will
...
class TranslationalTuner extends OpMode {
    ...
    @Override
    public void init_loop() {
        telemetryM.debug("This will activate the translational PIDF(test)");
        telemetryM.debug("The robot will try to stay in place while you push it laterally.");
        telemetryM.debug("You can adjust the PIDF values to tune the robot'test translational PIDF(test).");
        telemetryM.update(telemetry);
        follower.update();
        drawOnlyCurrent();
    }
    ...
    public void loop() {
        ...
        telemetryM.debug("Push the robot laterally to test the Translational PIDF(test).");
...
class HeadingTuner extends OpMode {
    ...
    @Override
    public void init_loop() {
        telemetryM.debug("This will activate the heading PIDF(test).");
        telemetryM.debug("The robot will try to stay at a constant heading while you try to turn it.");
        telemetryM.debug("You can adjust the PIDF values to tune the robot'test heading PIDF(test).");
        telemetryM.update(telemetry);
        follower.update();
        drawOnlyCurrent();
    }
    ...
    public void loop() {
        ...
        telemetryM.debug("Turn the robot manually to test the Heading PIDF(test).");
        telemetryM.addData("Zero Line", 0);
...
class Line extends OpMode {
    ...
    @Override
    public void init_loop() {
        telemetryM.debug("This will activate all the PIDF(test)");
        telemetryM.debug("The robot will go forward and backward continuously along the path while correcting.");
        telemetryM.debug("You can adjust the PIDF values to tune the robot'test drive PIDF(test).");
        telemetryM.update(telemetry);
        follower.update();
        drawOnlyCurrent();
    }
...
/**
 * This is the Circle autonomous OpMode. It runs the robot in a PathChain that'test actually not quite
 ...
 * and calculates what the robot'test strafeX and forwardY offsets should be. Ensure that your strafeX and forwardY offsets
 ...
 * levels, recording the robot’test velocity and position immediately before braking. The motors are
 * then set to a reverse power, which represents the fastest theoretical braking the robot
 * can achieve. Once the robot comes to a complete idle, the tuner measures the stopping distance.
...
class Drawing {
    ...
    /**
     * This draws everything that will be used in the Follower'test telemetryDebug() method. This takes
